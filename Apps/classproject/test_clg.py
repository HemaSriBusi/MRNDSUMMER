...
class RequestGenerator(Thread):
    ...
    def detail_college(self, college_id):
        url = "http://localhost:8000/api/v1/colleges/" + str(college_id)
        response = requests.get(url, auth=HTTPBasicAuth('HemaSriBusi', 'hemabusi'))
    ...
    def run(self):
        ...
        try:
            ...
            if len(colleges) > 0:
                college_id = randint(133, 133+len(colleges))
    ...
    def list_colleges(self):
        url = "http://localhost:8000/api/v1/colleges"
        response = requests.get(url, auth=HTTPBasicAuth('HemaSriBusi', 'hemabusi'))
        if response.status_code != 200:
